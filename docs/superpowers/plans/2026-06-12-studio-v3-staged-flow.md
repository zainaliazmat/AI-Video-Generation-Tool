...
## M5 BUILD AMENDMENTS (2026-06-13)

M5 shipped on `studio-v3-staged-flow` (13 commits, tip aa4ee6f): 560 backend green, tsc ×3 clean, remotion vitest green. Exit gate ran as a 4-dimension review WORKFLOW (T6 spec, T7 spec, cross-task holistic, quality) → adversarial verify each finding → synthesize: **17 findings confirmed** (2 critical, 9 important, 6 minor), verdict PASS_WITH_FIXES, all folded across 5 fix commits. PLUS a live curl E2E (real DeepSeek/Pexels/TTS, sid v3-f6a3a05…): start→approve script→approve voice→scenes gate; hero auto-fill flowed rank-1 backgroundClip into spec.json; a background pick swapped src rank-1→rank-2 on disk (distinct md5s, instant frontier re-derive, gate stayed awaiting); hero-pick guard returned HTTP 400. Evidence in ~/Downloads/studio-v3-eyes-on/m5-e2e-transcript.txt. Then a 2-agent adversarial re-verification of the two criticals (Fix A clean; Fix B surfaced a poisoning defect, fixed in aa4ee6f).

Ruled deviations / facts M6/M7 must honor:

1. **Eligibility is beat DATA+POSITION, not rendered props (CRITICAL fix dfb4913).** `eligible_templates(beat_data, position, scene_count, catalog)`: scene always; stat iff value+label; enumeration iff items (LIVE day one, stat-wins mutual exclusion); hook iff pos==0; outro iff pos==last. The shipped T7 model validated current rendered props → only same-template ever passed → M6 chips would have been dead. M6-T7 reads `/state.eligibleTemplates` (now correct). `_pick_template` delegates to the same helper + bounds guard + position gate.
2. **Cross-template switch re-derives props in assemble** (dfb4913 + executors threads beats). Data-driven targets (stat/enumeration/hook/outro) re-derive from the beat via recipe prop builders; the enumeration detector is `consumes=="enumeration"` (NOT kind, both are kind="scene"). A hero→`scene` switch needs footage media the hero lacks → **rejected PRE-WRITE in `_pick_template`** (aa4ee6f) with the same message build_spec uses, so no poisoned override row. M6 template chips: offering 'scene' on a hero is honest-but-needs-a-footage-pick-first (surface that, or omit 'scene' for clip-less heroes in the UI).
3. **drop_beat reconciles ALL five scene_index-keyed tables** (CRITICAL fix ef5eea0) via `store.drop_scene_index` (DELETE dropped + decrement above, one txn, both rederive paths). pick_log scene_index SHIFTS (it's a live join key in build_state; seq preserves audit order) — ruled correct.
4. **broaden uses plan.title** (not ctx.topic) — matches the autopilot whiff-fallback. **Image backgrounds carry kind:'image'** through clip_value→assemble (M4 HeroBackdrop already branches on type). **bg re_query auto-follow honors the K-floor** (select_clip, not rows[0]).
5. **State surface bulk reads** (no N+1): `get_footage_candidates_all` + whole-session pick_log/overrides fetched once. **session_edit bg-pick response now echoes the background_override** (picked_rank/source/query) — so M6 CAN read bg-pick confirmation from the edit response OR /state (both honest now; the E2E-flagged null-provenance seam is closed).
6. **CLI** `session_edit.build_parser()`/`main(argv)` extracted (tests exercise the real parser). `--target footage|background`, `--broaden`, `--template` wired; edit route forwards target/broaden/template (incl. multipart upload target).
7. Repo hygiene note: live E2E leaves untracked `remotion/public/assets/voiceover_v3-*.wav` render artifacts — consider a `.gitignore` entry `remotion/public/assets/voiceover_*.wav` (operator's call; never committed by the build).

**M5 STATUS: DONE — exit gate green (560 tests, tsc ×3, workflow review + live E2E + adversarial critical re-verify all cleared).**

---

## M6 BUILD EXPANSION (2026-06-13 — bite-sized, from a 4-agent ground-truth pass)

Expanded from: the frontend architecture map, the rev-2 mock digest (`~/Downloads/studio-v3-staged-flow-mock (1).html`), the approved §4.1 wireframe (`~/.gstack/projects/AIVideoGenerationTool/designs/blast-radius-sheet-20260612/`), and the live `/state` data contract (sid `v3-f6a3a05…`). Build order is dependency-driven; pages touch mostly-disjoint files but ALL import the Foundation, so Foundation lands first, sequentially. Exit gate: vitest + tsc ×3 green; in-browser eyes-on (gstack browse) of every gate + the §4.1 sheet; a11y checklist verified in DOM.

### Data contract (what M6 reads — pinned)
- `GET /api/session/[id]/state`: `{sid, scenes[], gates, autoRun}`. **Partial pre-spec** = `scenes:[]` (NOT 404) — discriminate by `scenes.length`. `gates` = `{gate:{state:'awaiting_approval'|'approved'|'reopened'|'stale', approved_at:string|null}}`; reopened-vs-frontier = `approved_at` truthy on an awaiting gate (token "now", not a real time). Per-scene: index, template, needsFootage, beatText, durationInFrames, candidates[], provenance{source,query,rank,pexelsId,pexelsUrl}|null, **eligibleTemplates[]** (beat-data+position), templateOverride{value,source,pickedRank}|null, **backgroundPool{rows[],poolError}**, **backgroundProvenance{source,pickedRank,query,pexelsId,pexelsUrl,updatedAt}|null** (null=gradient), **pickLogCount**, **lastPick{autoRank,humanRank}|null**.
- Script payload (`GET .../script`): {title, beats[{index,text,data,keywords,source,flag,flagReason}], sources[], verifyReport[], factFloor{supported,total,level}, **bandMiss|null**}.
- SSE (start+approve): `{type:'sid',sid}` (start only, FIRST) · `{type:'stage',stage,state:'running'|'done'|'failed',elapsed_s?,error?}` · `{type:'done',sid?,gates}` · `{type:'error',error|message}`. 409 single-flight by sid on approve. failed only fires for a RUNNING stage.
- Edit `POST .../edit`: `{op:'pick',scene,rank,target?}` · `{op:'re_query',scene,query|broaden:true,target?}` · `{op:'pick_template',scene,template}` · multipart `{op:'upload',scene,file,target?}`. Response `{ok,sid,scene,selectedRank,provenance}` — background pick now echoes the bg override (M5 amend 5). pick_template hero→scene rejected pre-write (400).

### File map (ADD vs REUSE)
- ADD: `lib/sse.ts` (shared reader, dedupes page.tsx+RenderControls), `lib/studio.ts` session client additions, `components/GateStepper.tsx`, `components/GateInterstitial.tsx`, `components/BlastRadiusSheet.tsx`, `components/StatefulStamp.tsx`, `components/StatusPills.tsx`, `components/ScenePlayer.tsx`+`ScenePlayerClient.tsx`, `app/video/[id]/scenes/page.tsx` (+ sub-components), `/footage`→`/scenes` redirect, `app/api/session/[id]/set-auto-run/route.ts` + `app/api/session/[id]/preview-reopen/route.ts` (small JSON), maybe `app/api/session/[id]/edit` already exists.
- REUSE/extend: `app/page.tsx` (T1), `video/[id]/{page,layout,script,voice,assemble}/page.tsx`, `components/{GateHeader,PipelineStepper,PreviewRail,MobilePiP,PlayerClient,VideoPlayer,RenderControls,HistoryList,ui}.tsx`, `globals.css`, `tailwind.config.ts`. `/api/generate` survives (OV-15).

### FOUNDATION (build first, sequential)
- **F1 — Design system.** `globals.css`: add `--warn:#e8a33d` + `--warn-soft:rgba(232,163,61,.14)` tokens; `body.ember` palette overrides (sibling-layer: `.heroB`/`.glow` gradients flip orange — the rail/players re-paint via CSS cascade, NOT props); a stale-dot amber-glow keyframe that collapses under `prefers-reduced-motion`. `tailwind.config.ts`: `warn`/`warn-soft` colors. `ui.tsx`: `Button` gains a `warn` variant (amber fill `#e8a33d`, text `#1a1308`); `Badge` already has amber. Acceptance: tokens resolve, ember class flips a test element, reduced-motion kills the glow; vitest+tsc.
- **F2 — Data layer.** `lib/sse.ts`: extract the SSE reader (getReader + TextDecoder + `\n\n` split + `data:` parse + typed dispatch over the SseEvent union). `lib/studio.ts`: `studio.session.{start(topic,{autoRun,targetLength})→stream, approve(id,gate,{voice,speed})→stream, state(id)→StateFull|StatePartial (typed), setAutoRun(id,flag), previewReopen(id,gate)}`; typed State/Scene/Gate interfaces (per the data contract). Two small JSON routes if absent: set-auto-run (→ `session_gate.py --op set_auto_run`), preview-reopen (→ `--op preview_reopen`). Acceptance: vitest on the SSE parser (feed canned frames, assert dispatch); tsc.
- **F3 — Shared components + scenes-route stub.** `GateStepper.tsx` (4 gate steps Script→Voice→Scenes→Assemble; states done/current/pending/**stale**; amber glowing stale dots on downstream pips; navigable completed; auto-run toggle slot; `aria-disabled` unreached). `StatefulStamp.tsx` (pre/post-approval stamp text per gate, mock copy). `StatusPills.tsx` (TWO slots MEDIA+TEMPLATE; MEDIA derive from provenance/backgroundProvenance/templateOverride with precedence pinned>uploaded>re-queried>auto; tones reuse the badge map, uploaded=green; TEMPLATE only when overridden). `GateInterstitial.tsx` (consumes approve SSE: per-stage task cards w/ ticking elapsed + leave-copy, failed card+verbatim error+ONE Retry re-posting approve, reconnected mode polling `/state`, `aria-live=polite`). `BlastRadiusSheet.tsx` (the approved hybrid: task-chips w/ time subtext + conditional pin-fate rows + amber Reopen; centered glass ≥sm / bottom-sheet <sm; focus-trap initial focus on Cancel; ESC=Cancel; fed by previewReopen). `ScenePlayer.tsx`+`ScenePlayerClient.tsx` (Player bound to a scene span via initialFrame+scoped durationInFrames, loops within span). Create a minimal `scenes/page.tsx` stub + `/footage`→`/scenes` redirect so GateHeader CHAIN can flip safely. Acceptance: vitest on StatusPills derivation + GateStepper state rendering + the BlastRadiusSheet conditional pin-rows; tsc.

### PAGES (after Foundation; each its own commit + two-stage review)
- **T11 — GateHeader + Hub.** `GateHeader.tsx`: CHAIN→`[script,voice,scenes,assemble]`; `gateState` prop gates the Next→ link (`aria-disabled` when unreached); amber Re-approve variant. `video/[id]/page.tsx`: gate cards speak v3 states (locked/stale/approved/building from `/state.gates`); Footage card→Scenes; timing-explainer row stays; post-Re-approve frontier toast. `HistoryList.tsx`: handle title-less stub metas (building / "draft — script failed" + Retry/Delete).
- **T1 — Topic.** `app/page.tsx`: length preset chips (30/60/180/300, hints per the mock LENHINT map), auto-run toggle (consequence label); Generate→`studio.session.start` SSE→navigate to `/video/[sid]/script` on the `sid` event (not done); failed start→"draft — script failed" hub state. `/api/generate` route untouched.
- **T3 — Script gate.** `script/page.tsx`: v2 beat cards + inline edit + flag-only re-verify + drop + feedback chips + regenerate + style memory (reposition, keep); StatefulStamp (pre: "script ready…/nothing downstream has run yet"; post via crumb: "approved · downstream builds from v1 · editing reopens this gate"); credibility badge from factFloor; **bandMiss warn pill** ("asked for N–M beats, got K — regenerate or approve as-is"); Approve→`studio.session.approve(id,'script')` SSE→GateInterstitial (replaces the v2 synchronous approve).
- **T4 — Voice gate.** `voice/page.tsx`: cards play YOUR beat 1 (M3 preview op, sid unconditional, spinner first-play, reuse path for replay); speed slider 0.8–1.2 w/ re-times note; gate copy replaced ("Approve locks the voice, then builds narration, timing, footage, and a first assembly — a few minutes"; DROP the false v2 "footage picks are kept" line); Approve→`approve(id,'voice',{voice,speed})` SSE→heavy interstitial.
- **T5 — Scenes accordion shell + arrival.** `scenes/page.tsx`: one row per scene, ONE open at a time; collapsed row = index · kind chip · truncated beat · duration · TWO StatusPills · chevron; collapsed bodies `display:none`+`inert`; arrival (all collapsed, rail auto-plays full assembly once from 0, one-time eyebrow "first assembly ready — every scene is editable below"). honest banner copy from the mock.
- **T6 — Per-scene player + mount budget.** ScenePlayer in the open row bound to the scene span; desktop rail auto-pauses while a scene plays; mobile MobilePiP UNMOUNTS while a row is open (lift `openSceneIndex` into `layout.tsx` context; both PreviewRail+MobilePiP subscribe). vitest asserts ≤2 players mounted.
- **T7 — Scenes controls column.** template chips (eligible-only from `eligibleTemplates`, AI pick tagged auto, pick=pin via pick_template; omit/disable 'scene' for clip-less heroes per M5 amend 2); footage pool (rank badges, rank1=auto); background pool with **tile 0 = gradient floor** (pickable) + policy label ("AI picked #1 · topic is filmable" / "AI kept the gradient — abstract beat"); Suggest/Re-query(broaden)/Upload row; empty-pool + quota(poolError) states; provenance popover (pickLogCount + lastPick auto→human); transition chips (fade/slide/none via patch); under-player strip (frames a–b · n.n s, verify chip, Fix-a-word, captions on/suppressed).
- **T8 — Assemble gate slimmed.** `assemble/page.tsx`: REMOVE per-scene controls (live in Scenes; chat whitelist still patches them); theme cards (Ember flips rail live via `body.ember` toggle); director chat + history/revert (v2); Render w/ progress; done-state demotes Render bar to a single tinted Download CTA (+ New video).
- **T9 — §4.1 reopen pattern (cross-cutting).** StatefulStamps everywhere; **BlastRadiusSheet on first edit-intent** at a completed gate (beat textarea Save / pool tile / template chip / voice card click → sheet with POST withheld; Cancel = zero backend traffic); after Reopen edits accumulate silently; gate's tinted action becomes amber Re-approve→ONE approve SSE; **pending-pick affordance** at a reopened Scenes gate (picked tile "pending · applies on Re-approve" ring+pill, player keeps old clip); stepper stale dots from `/state.gates`. Render-done projects follow the same pattern.
- **T10 — Responsive + a11y pass.** accordion stacks on mobile (player above controls); one tinted action per screen audit (Generate/Approve×3/Render/Re-approve); keyboard+SR labels on pool tiles, voice play, stepper steps; sheet bottom-sheet-below-sm + focus/ESC + ≥44px chevron targets; reduced-motion.

### Coordination notes
- Foundation's scenes-route stub + redirect lands BEFORE T11's CHAIN flip (else the Next link 404s).
- The GateInterstitial is shared by T1 (start), T3/T4 (approve), T5-scenes (voice→scenes heavy segment) — build it once in Foundation, reuse.
- bg-pick confirmation readable from the edit response OR `/state` (both honest, M5 amend 5).
- Mock ambiguities to design in M6 (not in the mock): `uploaded` pill (green+filename), stale-stepper treatment (amber glow — now specced by the wireframe), post-approval stamp flips for voice/scenes, the real director-chat loop (backend-driven, exists).

**M6 STATUS: EXPANDED — building Foundation first.**

---

## M6 BUILD AMENDMENTS (2026-06-13 — ruled during the build; M7 + wrap-up must honor these)

M6 shipped on `studio-v3-staged-flow` (Foundation F1–F3c + T11/T1 from the prior session; this session: T2-chrome → T3 → T4 → T5+T6+T7 → T8 → T9 → T10 + exit-gate fixes, ~10 commits c40ecdd→443c9a9). Exit gate: **224 preview vitest + 560 backend + 113 remotion green; tsc ×3 clean (preview/remotion/templates); a code-review pass (3 high-confidence bugs found + adversarially verified + fixed); an in-browser BUILD + static-chrome eyes-on PASS** (home + gate chrome screenshotted on a live dev server; `/footage`→`/scenes` 307; home/script/scenes routes 200). Ruled deviations / facts M7 + the operator must honor:

1. **The standalone "T2 stepper-bar page" was dropped from the expansion's PAGES list; the GateStepper (F3a) is mounted via a new `components/VideoChrome.tsx` client wrapper in the video layout.** VideoChrome is the single source of truth for the session's `{gates, autoRun, openSceneIndex}` (a React context every gate page + the rail + PiP consume), owns the `/state` poll, drives the **building-poll** for the still-generating script (ruling 2), and renders the stepper only on the 4 gate pages (current derived from the pathname; null on hub/running). The global `Nav` is **hidden on `/video/` routes** (a second fixed bar would collide); the stepper's Topic ✦ step + a new hub back-crumb link Home. `currentGateFromPath` is unit-tested.
2. **Gate-segment timing the interstitials encode (from `gates.GATE_SEGMENTS`):** approve(script)→voice segment is `[]` (near-instant, ~8 s python spawn; auto-run cascades through everything); approve(voice)→scenes segment is the HEAVY `[voice,timing,footage,assemble]`; approve(scenes)→assemble segment is `[]`. So the topic→script transition shows a building card, script-approve shows an empty/leave-copy card, **voice-approve shows the 4-stage heavy interstitial**, scenes-approve advances to assemble.
3. **Style-memory edit seeding is preserved via a fire-and-forget `session_script.approve`** (a pure `style_memory.json` fold, no gate/spec mutation — verified) fired alongside the gatekeeper SSE approve, because `gatekeeper.approve` does NOT carry edits. The accumulated before/after pairs fold once on script approve.
4. **Backend addition (M6 is otherwise frontend):** `session_gate.py` gained a `set_voice` op (→ `api.gate_set_voice` → `gatekeeper.set_voice`, both already existed + tested) + a `/api/session/[id]/set-voice` JSON route + `studio.session.setVoice`, to expose the voice §4.1 reopen (selecting a voice / committing a speed change at an approved gate → `set_voice` reopens the gate; the amber Re-approve then pays once). Other gates' reopens already route through `api.edit`→gatekeeper (script via `session_script`, scenes via `session_edit`); transitions are the exception (see 5).
5. **Transitions patch via the assemble path (`/scenes/{i}/transition`, spec_patch whitelist), NOT `session_edit`** — so they apply IMMEDIATELY and do **not** reopen the scenes gate (they're assemble-stage edits with their own history/revert). Footage/template/background picks DO reopen (gatekeeper defer). The Scenes controls reflect this: `run()` is §4.1-gated; transitions use `runNow()`.
6. **No clear-to-gradient backend op exists.** The background pool's "tile 0 = gradient floor" is rendered as the SELECTED-when-`backgroundProvenance===null` indicator (the floor), not a destructive action — picking a clip overrides; reverting to gradient is not wired. Flagged for a future backend op if the operator wants it pickable both ways.
7. **Collapsed accordion bodies are LAZY-rendered (only the open row mounts its controls + ScenePlayer), not `display:none`+`inert`.** This is the performant + a11y-correct accordion pattern (button `aria-expanded`/`aria-controls`; collapsed content simply absent for SRs) and is what enforces the ≤2-player mount budget (`lib/playerBudget.ts`, unit-tested) — rendering 50 hidden pools+players would blow it. Deviation from the mock's literal "display:none + inert" wording; same UX, better perf, valid a11y.
8. **Build smoke caught a LATENT app-breaker tsc+vitest can't see (fixed):** `lib/projects.ts` imported `node:path` and was pulled into the BROWSER bundle via `HistoryList`'s value-import of `discriminateStub` ← `app/page.tsx` → every project-library page 500'd (`UnhandledSchemeError: node:path`). Latent since T1/T11. Split the filesystem helpers into `lib/projects-server.ts`; `lib/projects.ts` is now client-safe. **Lesson: the dev-server build smoke is the only gate that catches webpack `node:` scheme leaks — run it.** (`import type` callers — TemplateGallery/Drawer/GuideToc — never leaked.)
9. **Code-review pass fixed 3 high-confidence bugs (all in shared components):** GateInterstitial's 409 reconnect poll fired on the first poll (an already-approved upstream gate satisfied the loose check) → now baselines the gate dict + fires on change; GateInterstitial Retry could race a still-draining run's SSE handler → generation-token guard (`genRef`/`alive()`, also fixes post-unmount writes); `app/page.tsx`'s `if(!generating)` guard read a stale closure → `mountedRef`.
10. **Per-screen tinted-action audit (one each):** Topic=Generate · Script=Approve/Re-approve(amber)/Approved✓ · Voice=same · Scenes=Approve(frontier→assemble)/Re-approve(amber)/none-when-approved · Assemble=Render (terminal; the chat patch "Apply & re-render" is contextual). The scenes frontier Approve was a gap found in T10 and added.
11. **Eyes-on split:** the BUILD + static-chrome eyes-on is DONE (this session). The full interactive pixel/motion review on a LIVE session (clicking a real topic through all gates, the §4.1 sheet, the per-scene player, the ember flip, motion) is the **operator's M7 gate** — it needs a real DeepSeek/Pexels/TTS run, which the agent did not execute. Artifacts owed per M7-T1/T2.

**M6 STATUS: BUILT — automated exit gate green (224+560+113 tests, tsc ×3, code-review, build+chrome eyes-on). Live-session pixel/motion eyes-on = operator's M7 gate.**
